...
async function fetchDirectBlockChildren(blockId, token, fetchImpl) {
  const children = [];
  let startCursor = "";
  do {
    const response = await fetchImpl(
...
function monthBounds(month) {
  const [year, monthNumber] = clean(month).split("-").map(Number);
  if (!year || !monthNumber) return null;
  const start = isoFromParts(year, monthNumber, 1);
  const endDate = new Date(Date.UTC(year, monthNumber, 0));
  const end = isoFromParts(endDate.getUTCFullYear(), endDate.getUTCMonth() + 1, endDate.getUTCDate());
  return { start, end };
}

function weekOverlapsMonth(week, month) {
  const bounds = monthBounds(month);
  if (!bounds || !week?.startDate || !week?.endDate) return false;
  return week.startDate <= bounds.end && week.endDate >= bounds.start;
}

async function fetchDailyWorkMonthBlocks(pageId, token, fetchImpl, options = {}) {
  const month = options.month || clean(options.today).slice(0, 7) || new Date().toISOString().slice(0, 7);
  const topLevelBlocks = await fetchDirectBlockChildren(pageId, token, fetchImpl);
  const selectedWeekBlocks = topLevelBlocks.filter((block) => {
    const week = parseWeekRange(`# ${blockText(block)}`);
    return week && weekOverlapsMonth(week, month);
  });

  const blocks = [];
  for (const block of selectedWeekBlocks) {
    const next = { ...block };
    next.children = block.has_children ? await fetchBlockChildren(block.id, token, fetchImpl) : [];
    blocks.push(next);
  }
  return blocks;
}
...
export async function fetchDailyWorkBlocks(options, dependencies = {}) {
  ...
  const month = options.month || clean(options.today).slice(0, 7);
  const blocks = await fetchDailyWorkMonthBlocks(pageId, token, fetchImpl, { month, today: options.today });
...
function pageToTask(page) {
  ...
  const status = normalizeStatus(properties.Status?.status?.name);
  const lastEditedTime = clean(page.last_edited_time);
  return {
    taskName: propertyTitle(properties["Task Name"]),
    workstream: clean(properties.Workstream?.select?.name),
    ...
    sourceDate: recordTime.slice(0, 10),
    recordTime,
    ...
    dashboardRank: clean(properties["Dashboard Rank"]?.select?.name),
    completedDate: status === "Done" ? lastEditedTime.slice(0, 10) : "",
...
export async function notionWorkflowSource(options, dependencies = {}) {
  ...
  let parsed;
  try {
    const blocks = await fetchDailyWorkMonthBlocks(options.dailyWorkPageId, token, dependencies.fetchImpl || fetch, {
      month,
      today,
    });
    ...
    if (/rate limit|rate_limited|limited/i.test(error?.message || "")) throw error;
    parsed = null;
  }
