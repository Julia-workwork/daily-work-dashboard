...
function summarizeWeek(week) {
  ...
  for (const item of [...completed, ...open]) {
  ...
  const categorySummary = [...categoryCounts.entries()]
    ...
    .join("；");
  const outcome = completed.slice(0, 8).join("；");
  const followUps = open.slice(0, 8).join("；");
...
export function parseDailyWorkMarkdown(markdown, options = {}) {
  ...
  const selectedWeeks = weeks.filter((week) => week.days.some((day) => day.date.startsWith(month)));
  const dailyExtracts = [];
  const tasks = [];
  const weeklyReviews = [];
...
export function parseDailyWorkBlocks(blocks, options = {}) {
  ...
  for (const weekBlock of blocks || []) {
    ...
    for (const dayBlock of weekBlock.children || []) {
      const dayName = dayBlock.type?.startsWith("heading") ? blockText(dayBlock) : "";
      if (!dayName) continue;
      const currentDay = {
        weekday: dayName,
        date: dateForDay(currentWeek, dayName),
