...
test("task board supports synced this month ongoing work separately from weekly ongoing work", async () => {
  ...
  assert.match(source, /isMonthlyOngoingActiveInMonth\(task,\s*monthKey\)/);
  assert.match(source, /isWorkflowMonthlyOngoingTask\(task\)[\s\S]*isMonthlyOngoingActiveInMonth\(task,\s*monthKey\)[\s\S]*taskMonthKey\(task\) === monthKey/);
...
test("monthly ongoing keeps every item in a compact expandable sorted list", async () => {
  ...
  assert.match(source, />01 Top</);
  assert.match(source, />02 High</);
  assert.match(source, />03 Normal</);
  ...
  assert.match(styles, /border-left-color:\s*#f5a623/);
  assert.match(styles, /border-left-color:\s*#64748b/);
